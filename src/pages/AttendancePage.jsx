...
function normalise(a) {
  return {
    ...
    studentName: a.first_name ? `${a.first_name} ${a.last_name}` : (a.studentName ?? ""),
...
export default function AttendancePage({ auth, students, attendance, setAttendance, canEdit, toast, feeBlocked = false, onGoFees}) {
  ...
  const classStudents = useMemo(
    () => students.filter(s => (s.className ?? s.class_name) === cls && s.status === "active"),
    [students, cls]
  );

  useEffect(() => {
    if (showBulk) setBulk(classStudents.map(s => ({ studentId: s.id ?? s.student_id, status: "present" })));
  }, [showBulk, classStudents]);

  const normalised = useMemo(() => attendance.map(a => a.attendance_id ? normalise(a) : a), [attendance]);
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        <Badge text={`Present: ${normalised.filter(a => a.status === "present").length}`} tone="success" />
        <Badge text={`Absent: ${normalised.filter(a => a.status === "absent").length}`} tone="danger" />
        <Badge text={`Late: ${normalised.filter(a => a.status === "late").length}`} tone="warning" />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 8, marginBottom: 10 }}>
        ...
        <Btn variant="ghost" onClick={() => { csv("attendance.csv", ["Date","Class","Student","Status"], filtered.map(a => [a.date,a.className,a.studentName,a.status])); toast("Attendance CSV exported","success"); }}>Export CSV</Btn>
        {canEdit && <Btn onClick={() => setShowBulk(true)}>Bulk Mark Class</Btn>}
      </div>
      {filtered.length === 0 ? <Msg text="No attendance records." /> : (
        <>
          <div style={{ overflowX: "auto" }}>
            <Table
              ...
              rows={rows.map(a => [
                a.date, a.className,
                <span key={a.id} style={{ color: C.text, fontWeight: 600 }}>{a.studentName}</span>,
                <Badge key="s" text={a.status} tone={a.status === "present" ? "success" : a.status === "late" ? "warning" : "danger"} />,
                <div key="x" style={{ display: "flex", gap: 6 }}>
                  {canEdit && <Btn variant="ghost" onClick={() => setEditing(a)}>Edit</Btn>}
                  {canEdit && <Btn variant="danger" onClick={() => del(a.id)}>Delete</Btn>}
                </div>
              ])}
            />
          </div>
      ...
      {showBulk && (
        <Modal title="Bulk Attendance by Class" onClose={() => setShowBulk(false)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Field label="Class"><select style={inputStyle} value={cls} onChange={e => setCls(e.target.value)}>{ALL_CLASSES.map(c => <option key={c}>{c}</option>)}</select></Field>
            <Field label="Date"><input type="date" style={inputStyle} value={date} onChange={e => setDate(e.target.value)} /></Field>
          </div>
          {classStudents.length === 0 ? <Msg text="No active students in this class." /> : (
            <div style={{ maxHeight: 320, overflowY: "auto", border: `1px solid ${C.border}`, borderRadius: 10, padding: 8, marginBottom: 10 }}>
              {classStudents.map(s => {
                ...
                return (
                  <div key={sid} style={{ display: "grid", gridTemplateColumns: "1fr 150px", gap: 8, alignItems: "center", borderBottom: `1px solid ${C.border}`, padding: "8px 4px" }}>
                    <div style={{ color: C.text }}>{name}</div>
                    <select style={inputStyle} value={val} onChange={e => setBulk(prev => prev.map(x => x.studentId === sid ? { ...x, status: e.target.value } : x))}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            ...
            <Btn onClick={saveBulk}>Save Class Attendance</Btn>
          </div>
        </Modal>
      )}
      {editing && (
        <Modal title="Edit Attendance" onClose={() => setEditing(null)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <Field label="Class"><input style={inputStyle} value={editing.className} disabled /></Field>
            <Field label="Date"><input type="date" style={inputStyle} value={editing.date} onChange={e => setEditing({ ...editing, date: e.target.value })} /></Field>
            <Field label="Status">
