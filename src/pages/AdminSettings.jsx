...
// ─── SCHOOL INFO TAB ──────────────────────────────────────────────────────────
const SchoolInfoTab = ({ onSave, auth, onSchoolSaved }) => {
  ...
  const loadSchoolData = async () => {
    ...
    try {
      ...
      setForm(prev => ({
        ...
        academic_year: data.academic_year || data.year || prev.academic_year,
        ...
        library_shortcode: data.library_shortcode || prev.library_shortcode,
      }));
    ...
    try {
      ...
      const saveRes = await apiFetch("/settings/school", {
        ...
        body: {
          ...
          library_shortcode: form.library_shortcode,
        },
        token: auth?.token,
      });
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 24px" }}>
        ...
        <Inp label="Library Shortcode" value={form.library_shortcode} onChange={f("library_shortcode")} placeholder="e.g. LIB-001" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "0 24px" }}>
        ...
        <Inp label="Academic Year" value={form.academic_year} onChange={f("academic_year")} placeholder="e.g. 2025" />
...
SchoolInfoTab.propTypes = { onSave: PropTypes.func.isRequired, auth: PropTypes.object, onSchoolSaved: PropTypes.func };
...
// ─── ADMIN SETTINGS PAGE ──────────────────────────────────────────────────────
export default function AdminSettings({ auth, initialTab, onPermissionsSaved, onSchoolSaved }) {
  ...
  const renderTab = () => {
    ...
    switch (activeTab) {
      case "school":        return <SchoolInfoTab onSave={save} auth={auth} onSchoolSaved={onSchoolSaved} />;
