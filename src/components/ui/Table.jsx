          ...
          <thead>
            <tr>
              ...
              {headers.map((header, index) => (
                <th key={`${header}-${index}`} style={stickyHeader ? { position: "sticky", top: 0, zIndex: 2, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" } : undefined}>{header}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              tableData.map((row, rowIndex) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      background: selected ? "color-mix(in srgb, var(--color-primary) 10%, transparent)" : striped && rowIndex % 2 === 1 ? "color-mix(in srgb, var(--color-bg-hover) 70%, transparent)" : undefined,
                      boxShadow: selected ? "inset 3px 0 0 var(--color-primary)" : undefined,
                  ...
                  >
                    ...
                    {renderRow ? renderRow(row, rowIndex) : headers.map((header, colIndex) => (
                      <td key={`${header}-${colIndex}`} style={{ padding: dense ? "10px 12px" : undefined }}>{getCell(row, header, colIndex)}</td>
                    ))}
                  </tr>
                );
              })
