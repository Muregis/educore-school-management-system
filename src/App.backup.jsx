...
export default function App() {
  ...
  useEffect(() => {
    if (school?.primary_color) {
      ...
      document.documentElement.style.setProperty(
        '--color-primary-hover',
        darkenColor(school.primary_color, 15)
      );
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", background:C.bg, color:C.text, fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      {isQRVerification ? (
        ...
        <div className="ec-topbar" style={{ height:"var(--topbar-height)", background:"var(--color-bg-surface)", borderBottom:"1px solid var(--color-border)", display:"flex", justifyContent:"space-between", alignItems:"center", padding:"0 var(--space-5)", position:"sticky", top:0, zIndex:"var(--z-sticky)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            {isMobile && (
              <button onClick={() => setDrawerOpen(true)} style={{ background:"transparent", border:"1px solid var(--color-border)", borderRadius:"var(--radius-md)", color:"var(--color-text-muted)", cursor:"pointer", padding:"var(--space-2) var(--space-3)", fontSize:"var(--text-lg)" }} className="touch-target">☰</button>
            )}
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {school.logo_url && !isMobile && (
                <img src={school.logo_url} alt="Logo" style={{ width: "28px", height: "28px", borderRadius: "var(--radius-sm)", objectFit: "cover" }} />
              )}
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:"var(--space-2)" }}>
                  <span style={{ fontSize:"var(--text-lg)" }}>{currentNav?.icon}</span>
                  <span style={{ fontWeight:"var(--font-extrabold)", fontSize: isMobile ? "var(--text-md)" : "var(--text-lg)", color:"var(--color-text-primary)" }}>{currentNav?.label || page}</span>
                </div>
                {!isMobile && (
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            {!isMobile && (
              <div style={{ background:`${roleColor}18`, border:`1px solid ${roleColor}44`, borderRadius:"var(--radius-lg)", padding:"var(--space-1) var(--space-3)", fontSize:"var(--text-xs)", fontWeight:"var(--font-bold)", color:roleColor, textTransform:"capitalize" }}>{auth.role}</div>
            )}
            {["admin","teacher","finance"].includes(auth.role) && (
              <div style={{ position:"relative" }}>
                <button onClick={() => setShowBell(!showBell)} style={{ border:"1px solid var(--color-border)", borderRadius:"var(--radius-md)", background:"var(--color-bg-card)", color:"var(--color-text-secondary)", cursor:"pointer", padding:"var(--space-2) var(--space-3)", fontSize:"var(--text-sm)", position:"relative" }} className="touch-target">
                  🔔
                  {notifications.filter(n => !n.read).length > 0 && (
                    <span style={{ position:"absolute", top:"var(--space-1)", right:"var(--space-1)", width:"8px", height:"8px", borderRadius:"var(--radius-full)", background:"var(--color-rose)", border:"2px solid var(--color-bg-surface)" }} />
                  )}
                </button>
            ...
            {isMobile && (
              <button onClick={handleLogout} style={{ width:"34px", height:"34px", borderRadius:"var(--radius-md)", background:`${roleColor}22`, border:`1px solid ${roleColor}44`, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:"var(--font-bold)", fontSize:"var(--text-sm)", color:roleColor, cursor:"pointer" }} title="Logout" className="touch-target">
