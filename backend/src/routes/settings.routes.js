...
router.get("/school", async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      academic_year: data.academic_year || data.year || settings.get("academic_year") || "",
...
// PUT /api/settings/school - update school profile fields
router.put("/school", requireRoles("admin", "director", "superadmin"), async (req, res, next) => {
  try {
    const { schoolId } = req.user;
    ...
    if (whatsapp_business_number !== undefined) updatePayload.whatsapp_business_number = whatsapp_business_number;
    const { data, error } = await supabase
      .from("schools")
